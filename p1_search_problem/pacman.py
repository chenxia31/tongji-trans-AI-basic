...
"""
Pacman.py拥有经典pacman游戏的逻辑以及运行游戏的主要代码。此文件分为三个部分:

(i)您与pacman世界的界面:Pacman是一个复杂的环境。
        您可能不想通读我们编写的所有代码，以使游戏正确运行。
        本节包含您需要了解的代码部分，以便完成项目。
        Game.py中也有一些你应该理解的代码。

(Ii)pacman的隐藏秘密:
    本节包含pacman环境用于决定谁可以移动到哪里,当东西碰撞时谁死亡等的所有逻辑代码。
    您应该不需要阅读这部分代码，但如果您愿意，您可以阅读。

(Iii)启动游戏的框架:
    最后一部分包含用于阅读您用于设置游戏的命令的代码，
    然后启动新游戏，以及链接所有外部部分（代理功能、图形）。
    查看此部分，查看所有可用的选项。
